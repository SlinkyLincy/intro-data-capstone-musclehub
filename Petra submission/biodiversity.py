...
protection_count = species.groupby('conservation_status')    .scientific_name.nunique().reset_index()    .sort_values(by='scientific_name')
...
ax = plt.subplot()
plt.bar(range(len(protection_count)), protection_count.scientific_name, color = 'darkgreen')
#ax.set_yscale('log')

ax.set_xticks(range(len(protection_count)))
ax.set_xticklabels(protection_count.conservation_status)
...
plt.figure(figsize = (10,4))
ax = plt.subplot()
plt.bar(range(len(protection_counts)), protection_counts.scientific_name, color = 'darkgreen')
#ax.set_yscale('log')
...
species['is_sheep'] = species.common_names.apply(lambda value: 'True' if 'Sheep' in value else 'False')
...
plt.bar(range(len(obs_by_park)), obs_by_park.observations, color = 'darkgreen')

ax.set_xticks(range(len(obs_by_park)))
ax.set_xticklabels(['Bryce', 'Great Smoky Mountains', 'Yellowstone', 'Yosemite'], fontsize = 14)
plt.xlabel('National Park', fontsize = 16)

ax.tick_params(labelsize = 14)
plt.ylabel('Observations', fontsize = 16)
plt.title('Observations of Sheep per Week', fontsize = 20)

plt.savefig('sheep_bar.png')
plt.show()
...
# ## Additional Analysis for the Presentation

# List of National Parks and species observations

# In[80]:


parks = observations.groupby('park_name').observations.count().reset_index()
print(parks)


# List of sheep and non-sheep observations

# In[81]:


obs_by_park['not_sheep'] = 5824 - obs_by_park.observations
print(obs_by_park)


# Compare observations numbers between Yellowstone and Great Smoky Mountains

# In[82]:


contingency3 = [[507, 5317], [149, 5675]]
